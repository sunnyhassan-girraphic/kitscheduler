...
def timeline_view(request):
    ...
    # Build per-day availability strips for engines and laptops
        ...
        for asset in assets:
            ...
            for d in days:
                ...
                strips.append({
                    "date": d,
                    "booking": hit,
                    "color": hit.job.resolve_color() if hit else None,
                    "is_weekend": _is_weekend(d),
                    "is_today": d == today,
                })
            rows.append({"asset": asset, "strips": strips})
        return rows
...
def create_asset_booking(request):
    """Book an engine or laptop (non-license, non-kit asset) on a job."""
